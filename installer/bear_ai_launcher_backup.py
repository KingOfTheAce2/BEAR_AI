...
def start_web_interface(port: int = 3000):
    ...
    if package_json.exists():
        ...
        print("❌ Web interface not found. Please check installation.")
        return False
...
def start_api_server(host: str = "127.0.0.1", port: int = 8000):
    ...
    try:
        ...
        print(f"❌ Failed to start API server: Missing dependency {e}")
        print("   Run: python -m pip install -r installer/requirements-clean.txt")
        return False
    except Exception as e:
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "web":
            ...
            print("Available commands: web, api, gui, chat")
            return 1
    else:
